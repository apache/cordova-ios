...
function PodsJson (podsJsonPath) {
    ...
    if (filename !== PodsJson.FILENAME) {
        throw new CordovaError(`PodsJson: The file at ${this.path} is not \`${PodsJson.FILENAME}\`.`);
    }

    if (!fs.existsSync(this.path)) {
        events.emit('verbose', `pods.json: The file at ${this.path} does not exist.`);
...
PodsJson.prototype.__remove = function (kind, name) {
    if (this.contents[kind][name]) {
        ...
        this.__dirty = true;
        events.emit('verbose', `Remove from pods.json for \`${name}\` - \`%s\``);
    }
};
