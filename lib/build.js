...
module.exports.run = function (buildOpts) {
    ...
    return Promise.resolve()
        ...
        .then(() => {
            ...
            const xcodebuildArgs = getXcodeBuildArgs(projectPath, configuration, emulatorTarget, buildOpts, targetRuntime);
 ...
 */
function getXcodeBuildArgs (projectPath, configuration, emulatorTarget, buildConfig = {}, targetRuntime = null) {
    let options;
    let buildActions;
    let settings;
    ...
    if (buildConfig.device && !buildConfig.catalyst) {
        ...
        if (buildConfig.catalyst) {
            ...
            options = options.concat([
                ...
                '-destination', customArgs.destination || `platform=iOS Simulator,name=${emulatorTarget}${targetRuntime ? `,OS=${targetRuntime}` : ''}`
            ]);
        }
