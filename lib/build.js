...
const { getDeviceFromDeviceTypeId } = require('./simctlHelper');
...
module.exports.run = function (buildOpts) {
    ...
    // When target includes a runtime (e.g., "iPhone-16-Pro, 18.5"), capture it for xcodebuild destination
    const targetRuntime = buildOpts.target?.split(',')?.at(1)?.trim();
    let emulatorTarget = 'iOS Device';
    let emulatorDeviceId = null;
    ...
    return Promise.resolve()
        ...
        .then(() => {
            ...
            if (!buildOpts.device) {
                ...
                if (newTarget) {
                    ...
                    // Convert spaces to hyphens to match simulator identifiers (e.g., "iPhone 16 Pro" -> "iPhone-16-Pro")
                ...
                return promise.then(theTarget => {
                    if (!theTarget) {
                        ...
                        // Try to get the specific device UDID to handle cases with duplicate simulators
                        try {
                            if (buildOpts.target) {
                                const deviceTypeId = `com.apple.CoreSimulator.SimDeviceType.${theTarget.simIdentifier}${targetRuntime ? `, ${targetRuntime}` : ''}`;
                                const device = getDeviceFromDeviceTypeId(deviceTypeId);
                                if (device && device.id) {
                                    emulatorDeviceId = device.id;
                                    events.emit('log', `Building for "${emulatorTarget}" Simulator (UDID: ${emulatorDeviceId}).`);
                                } else {
                            ...
                            events.emit('warn', `Could not resolve device UDID: ${err.message}`);
                            events.emit('log', `Building for "${emulatorTarget}" Simulator (${theTarget.identifier}, ${theTarget.simIdentifier}).`);
                        }
        ...
        .then(() => {
            ...
            // Pass targetRuntime so simulator destination can pin the requested OS version
            const xcodebuildArgs = getXcodeBuildArgs(projectPath, configuration, emulatorTarget, buildOpts, targetRuntime, emulatorDeviceId);
 ...
 * @param  {String}  targetRuntime    The runtime version for the simulator (e.g., "18.5")
 * @param  {String}  emulatorDeviceId The device UDID for the simulator
 * @return {Array}                  Array of arguments that could be passed directly to spawn method
 */
function getXcodeBuildArgs (projectPath, configuration, emulatorTarget, buildConfig = {}, targetRuntime = null, emulatorDeviceId = null) {
    let options;
    let buildActions;
    let settings;
    ...
    if (buildConfig.device && !buildConfig.catalyst) {
        ...
        if (buildConfig.catalyst) {
            ...
            options = options.concat([
                ...
                '-destination', customArgs.destination || (emulatorDeviceId ? `platform=iOS Simulator,id=${emulatorDeviceId}` : `platform=iOS Simulator,name=${emulatorTarget}${targetRuntime ? `,OS=${targetRuntime}` : ''}`)
            ]);
        }
