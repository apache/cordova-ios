...
const handlers = {
    ...
    framework: { // CB-5238 custom frameworks only
        install: function (obj, plugin, project, options) {
            ...
            if (!custom) {
                ...
                if (keepFrameworks.indexOf(src) < 0) {
                    if (obj.type === 'podspec') {
                        ...
                        events.emit('verbose', `Adding non-custom framework to project... ${src} -> ${JSON.stringify(opt)}`);
                        project.xcode.addFramework(src, opt);
                        events.emit('verbose', `Non-custom framework added to project. ${src} -> ${JSON.stringify(opt)}`);
                    }
                }
                return;
            }
            ...
            events.emit('verbose', `Adding custom framework to project... ${src} -> ${JSON.stringify(opt)}`);
            project.xcode.addFramework(project_relative, opt);
            events.emit('verbose', `Custom framework added to project. ${src} -> ${JSON.stringify(opt)}`);
        },
        uninstall: function (obj, plugin, project, options) {
            if (isSwiftPackagePlugin(plugin)) return;
