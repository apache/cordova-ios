...
function startSimulator (device, options = {}) {
    ...
    const waitForDebugger = options?.waitForDebugger ?? false;
    ...
    simctl.launch(waitForDebugger, device.id, options.appIdentifier, []);
    simctl.extensions.log(device.id, options.logPath);
}
