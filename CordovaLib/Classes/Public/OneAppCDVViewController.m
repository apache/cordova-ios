...
@property (nonatomic, readwrite, strong) id <OneAppCDVWebViewEngineProtocol> webViewEngine;
...
@implementation OneAppCDVViewController
...
- (void)__init
{
    if ((self != nil) && !self.initialized) {
        _commandQueue = [[OneAppCDVCommandQueue alloc] initWithViewController:self];
        _commandDelegate = [[OneAppCDVCommandDelegateImpl alloc] initWithViewController:self];
...
- (void)loadSettings
{
    OneAppCDVConfigParser* delegate = [[OneAppCDVConfigParser alloc] init];
...
- (void)viewDidLoad
{
    ...
    if ([self.startupPluginNames count] > 0) {
        [OneAppCDVTimer start:@"TotalPluginStartup"];

        for (NSString* pluginName in self.startupPluginNames) {
            [OneAppCDVTimer start:pluginName];
            [self getCommandInstance:pluginName];
            [OneAppCDVTimer stop:pluginName];
        }

        [OneAppCDVTimer stop:@"TotalPluginStartup"];
    }
...
- (UIView*)newCordovaViewWithFrame:(CGRect)bounds
{
    ...
    if (NSClassFromString(webViewEngineClass)) {
        ...
        if (!self.webViewEngine || ![self.webViewEngine conformsToProtocol:@protocol(OneAppCDVWebViewEngineProtocol)] || ![self.webViewEngine canLoadRequest:[NSURLRequest requestWithURL:self.appUrl]]) {
    ...
    if ([self.webViewEngine isKindOfClass:[OneAppCDVPlugin class]]) {
        [self registerPlugin:(OneAppCDVPlugin*)self.webViewEngine withClassName:webViewEngineClass];
    }
