...
@interface OneAppCDVCommandQueue () {
...
@implementation OneAppCDVCommandQueue
...
{
    __weak OneAppCDVCommandQueue* weakSelf = self;
...
- (BOOL)execute:(OneAppCDVInvokedUrlCommand*)command
{
    ...
    OneAppCDVPlugin* obj = [_viewController.commandDelegate getCommandInstance:command.className];

    if (!([obj isKindOfClass:[OneAppCDVPlugin class]])) {
