...
@interface OneAppCDVWhitelistPattern : NSObject {
    @private
...
@implementation OneAppCDVWhitelistPattern
...
- (id)initWithScheme:(NSString*)scheme host:(NSString*)host port:(NSString*)port path:(NSString*)path
{
    ...
    if (self) {
        if ((scheme == nil) || [scheme isEqualToString:@"*"]) {
            ...
            _scheme = [NSRegularExpression regularExpressionWithPattern:[OneAppCDVWhitelistPattern regexFromPattern:scheme allowWildcards:NO] options:NSRegularExpressionCaseInsensitive error:nil];
        }
        if ([host isEqualToString:@"*"] || host == nil) {
            _host = nil;
        } else if ([host hasPrefix:@"*."]) {
            _host = [NSRegularExpression regularExpressionWithPattern:[NSString stringWithFormat:@"([a-z0-9.-]*\\.)?%@", [OneAppCDVWhitelistPattern regexFromPattern:[host substringFromIndex:2] allowWildcards:false]] options:NSRegularExpressionCaseInsensitive error:nil];
        } else {
            _host = [NSRegularExpression regularExpressionWithPattern:[OneAppCDVWhitelistPattern regexFromPattern:host allowWildcards:NO] options:NSRegularExpressionCaseInsensitive error:nil];
        }
        ...
        if ((path == nil) || [path isEqualToString:@"/*"]) {
            ...
            _path = [NSRegularExpression regularExpressionWithPattern:[OneAppCDVWhitelistPattern regexFromPattern:path allowWildcards:YES] options:0 error:nil];
        }
    }
    return self;
}
...
@implementation OneAppCDVWhitelist
    ...
    if ([origin isEqualToString:@"*"]) {
        ...
        if (m != nil) {
            ...
            if (scheme == nil) {
                ...
                [self.whitelist addObject:[[OneAppCDVWhitelistPattern alloc] initWithScheme:@"http" host:host port:port path:path]];
                [self.whitelist addObject:[[OneAppCDVWhitelistPattern alloc] initWithScheme:@"https" host:host port:port path:path]];
            } else {
                [self.whitelist addObject:[[OneAppCDVWhitelistPattern alloc] initWithScheme:scheme host:host port:port path:path]];
            }
...
- (BOOL)URLIsAllowed:(NSURL*)url logFailure:(BOOL)logFailure
{
    ...
    for (OneAppCDVWhitelistPattern* p in self.whitelist) {
        if ([p matches:url]) {
            return YES;
        }
    }
